...
def convert_tensor2np(img):
  img = img.permute(1, 2, 0)
  img = img.data.cpu().numpy() # Tensor -> numpy
  return img


def calculate_mse(img_gen, img_gt):
  """
    Calculate MSE: Compute the MSE.
  """
  img_gen, img_gt = convert_tensor2np(img_gen), convert_tensor2np(img_gt)
...
def calculate_psnr(img_gen, img_gt):
  """
    Calculate PSNR: Compute the peak signal to noise ratio error.
  """
  img_gen, img_gt = convert_tensor2np(img_gen), convert_tensor2np(img_gt)
...
def calculate_ssim(img_gen, img_gt):
  """
    Calculate SSIM: return the structural similarity.
  """
  img_gen, img_gt = convert_tensor2np(img_gen), convert_tensor2np(img_gt)
