...
class BaseNetGenerator(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, filters=32):
        ...
        self.activation = nn.ReLU()#論文曰く、ほとんどの３DconvではLeakyRelu, deconvでRelu
        ...
        self.deconv_5 = utils.deconv_421(self.filters*4, self.filters, self.activation)
        self.deconv_6 = utils.deconv_last(self.filters*2, self.out_dim)
    ...
    def forward(self, x):
        ...
        concat_4 = torch.cat([conv_2, deconv_4], dim=1) # -> [1, 128, 16, 32, 32]
        # utils.show_size(concat_4)

        deconv_5 = self.deconv_5(concat_4) # -> [1, 32, 32, 64, 64]
        # utils.show_size(deconv_5)
        concat_5 = torch.cat([conv_1, deconv_5], dim=1) # -> [1, 64, 32, 64, 64]
        # utils.show_size(concat_5)

        deconv_6 = torch.tanh(self.deconv_6(concat_5)) # -> [1, 3, 32, 128, 128]
...
class BaseNetDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO: 通常のGANのときは，行列の形変える
        conv_6 = conv_6.view(-1, 1).squeeze(1)
