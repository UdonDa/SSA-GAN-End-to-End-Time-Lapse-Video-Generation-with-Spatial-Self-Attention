...
class BaseNetGenerator(nn.Module):
    ...
    3DResBlockの後，空間・時間のAttention計算を追加している(See spatial_temporal_attention_block.py).
    ...
    def __init__(self, in_dim=3, out_dim=3, filters=64):
        ...
        for _ in range(6):
          ...
          residual_blocks.append(spatial_temporal_attention_block.SpatialTemporalAttentionBlock(self.filters * 4))
    ...
    def forward(self, x):
        ...
        f2 = self.conv_1(f1) # -> [1, 64, 16, 64, 64]
        # utils.show_size(f2)

        f3 = self.conv_2(f2) # -> [1, 128, 8, 32, 32]
        ...
        f5 = self.deconv_3(f4) # -> [1, 3, 32, 128, 128]
        # utils.show_size(f5)
        
        out_conv = self.out_conv(f5)
