...
class BaseNetGenerator(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=3, filters=64):
        ...
        self.conv_first = utils.conv_first_encdec_InstanceNorm(self.in_dim, self.filters, self.activation, self.use_spectral_norm) # 64

        self.conv_1 = utils.conv_421_InstanceNorm(self.filters, self.filters*2, self.activation, self.use_spectral_norm) # 64
        self.attn_hw_1 = self_attention_for_hw.SelfAttentionForHW(self.filters, self.activation)
        self.conv_2 = utils.conv_421_InstanceNorm(self.filters*2, self.filters*4, self.activation, self.use_spectral_norm) # 128
        ...
        self.deconv_2 = utils.deconv_421_InstanceNorm(self.filters*4, self.filters*2, self.activation, self.use_spectral_norm)
        self.attn_hw_4 = self_attention_for_hw.SelfAttentionForHW(self.filters*2, self.activation)
        self.deconv_3 = utils.deconv_421_InstanceNorm(self.filters*2, self.filters, self.activation, self.use_spectral_norm)
...
class BaseNetDiscriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_2 = utils.conv_421(self.filters, self.filters*2, self.activation, self.use_spectral_norm) # 64
        self.conv_3 = utils.conv_421(self.filters*2, self.filters*4, self.activation, self.use_spectral_norm) # 128
        self.conv_4 = utils.conv_421(self.filters*4, self.filters*8, self.activation, self.use_spectral_norm) # 256
        self.conv_5 = utils.conv_421(self.filters*8, self.filters*16, self.activation, self.use_spectral_norm) # 512
