...
parser.add_argument('--netG_S2', 
	default='./netG_S2_e_067.pth', 
	help="path to netG Stage 2")
parser.add_argument('--netG_S1', default='./netG_S1_030.pth', 
...
cuda = opt.cuda
# ======================================
# change the test data path here. 
root_path = '../sky_timelapse/sky_test'
# ========================================
...
# -------------set inputs---------------------------------
testset = VideoFolder(root=root_path, nframes = 32,
...
#subsampler = SubsetRandomSampler(perm)
valid_loader = DataLoader(testset,
					  batch_size=48,
					  num_workers=1,shuffle=True,drop_last = True, pin_memory=False)						 
valid_iter = iter(valid_loader)
val_gt, _ = valid_iter.next()
print('val video loaded.')
#if not os.path.exists('./valid_img.pt'):
#	torch.save(valid_img, 'valid_img.pt')
#valid_img = torch.load('valid_img.pt')
if cuda:
	netG_S1.cuda()
	netG_S2.cuda()
	val_gt = val_gt.cuda()
